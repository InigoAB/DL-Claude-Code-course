...
class TestRealIntegrationIssues:
    ...
    def test_vector_store_real_search_empty_results(self, real_config, sample_course_data):
        ...
        if real_config.MAX_RESULTS == 0:
            ...
            assert not results.is_empty(), "Search should return results with valid data"
    ...
    def test_course_search_tool_real_execution(self, real_config, sample_course_data):
        ...
        if real_config.MAX_RESULTS == 0:
            ...
            assert "No relevant content found" not in result, "CourseSearchTool failing due to MAX_RESULTS=0!"
        else:
            assert "No relevant content found" not in result
            assert "Test Course" in result
            assert len(search_tool.last_sources) > 0

    def test_course_search_tool_with_fixed_config(self, temp_chroma_path, sample_course_data):
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_rag_system_end_to_end_real_flow(self, mock_anthropic, real_config, sample_course_data):
        ...
        if real_config.MAX_RESULTS == 0:
            ...
            assert len(sources) == 0, "Sources empty due to MAX_RESULTS=0 causing search to return nothing"
        else:
